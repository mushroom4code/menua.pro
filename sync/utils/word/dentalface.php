...
$entity = mfa(mysqlQuery("SELECT * FROM `entities` WHERE `identities`='2'"));
if (!$entity) {
...
$data = [
	...
	/* ${ */ 'clientsPassportsDate' => date("d.m.Y", strtotime($passport['clientsPassportsDate'])), //} - организация выдавшая паспорт
...
//	'clientsPhonesPhone' => $phones, //} - телефонный номер клиента
	...
	/* ${ */ 'entitiesAddressUr' => $entity['entitiesAddressUr'], //} - адрес

	/* ${ */ 'entitiesOKPO' => $entity['entitiesOKPO'], //} -
...
for ($n = 0; $n < 13; $n++) {
	$data['og' . $n] = $entity['entitiesOGRN'][$n];
}
$rows = [];
