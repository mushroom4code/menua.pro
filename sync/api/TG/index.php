...
if (($_JSON['message']['chat']['type'] ?? false) == 'private') {//Общение через личку
	...
	if (($matches[0] ?? false) && strlen($matches[1] ?? '') == 5) {
		...
		if (!$client['clientsTG']) {
			...
			mysqlQuery("UPDATE `clients` SET"
					. " `clientsTG`= '" . $_JSON['message']['chat']['id'] . "',"
					. " `clientsTGset` = NOW()"
	...
	if (mb_strtolower(trim($_JSON['message']['text'] ?? '')) == 'стикер') {//Автосохранение стикеров
		$stickers = query2array(mysqlQuery("SELECT * FROM `TGstickers` WHERE `TGstickersAddedBy` IN(" . $user['idusers'] . ")"));
		$rand = rand(0, count($stickers) - 1);
		sendTelegram('sendSticker', ['chat_id' => $user['usersTG'], 'sticker' => $stickers[$rand]['TGstickersFile']]);
	}

	if (mb_strtolower(trim($_JSON['message']['text'] ?? '')) == 'мои стикеры') {//Автосохранение стикеров
		$stickers = query2array(mysqlQuery("SELECT * FROM `TGstickers` WHERE `TGstickersAddedBy` IN(" . $user['idusers'] . ")"));
		foreach ($stickers as $sticker) {
			sendTelegram('sendSticker', ['chat_id' => $user['usersTG'], 'sticker' => $sticker['TGstickersFile']]);
		}
	}
