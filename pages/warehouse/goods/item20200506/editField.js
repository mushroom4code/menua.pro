...
async function editField(inputData) {
	...
	if (['itemName', 'ballance', 'goodsBarcode', 'addBarcode', 'goodsUSUratio', 'goodsQty', 'goodsMinLimit', 'goodsMaxLimit'].indexOf(inputData.field) > -1) {
		var input = el('input');
		input.type = 'text';
		input.id = 'inputField';
	...
	if (inputData.field === 'ballance') {
		input.style.display = 'inline';
		input.style.width = 'auto';
		input.style.textAlign = 'right';
		input.size = '3';

		boxBody.appendChild(el('spab', {innerHTML: ` ${inputData.units}`}));
	}
	if (inputData.field === 'goodsUSUratio') {
		input.addEventListener('input', function () {
