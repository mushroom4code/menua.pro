...
//Добавляем продажу
	mysqlQuery("INSERT INTO `f_sales` SET "
			. "`f_salesNumber` = (SELECT * FROM (SELECT IF(isnull((SELECT MAX(f_salesNumber) FROM `f_sales` WHERE `f_salesClient`='" . $client['idclients'] . "' AND `f_salesEntity`='" . intval($_JSON['sale']['entity']) . "' AND NOT isnull(`f_salesIsAppendix`))),2,(SELECT MAX(f_salesNumber) FROM `f_sales` WHERE `f_salesClient`='" . $client['idclients'] . "' AND NOT isnull(`f_salesIsAppendix`))+1)) as `tmp`),"
...
//биндим выполненные процедуры если они есть.
			...
			if ($service['price'] ?? 0) {//подготавливаем данные для эвотора, наверное уже лишнее, но пускай пока будет. Может по аналогии надо будет со СБИС делать.
				...
				$SBIS['nomenclatures'][] = [
					...
					"priceNomenclature" => ($service['price'] ?? 0),
					"quantityNomenclature" => ($service['qty'] ?? 0),
					"measureNomenclature" => "шт",
					"kindNomenclature" => "у",
					"totalPriceNomenclature" => ($service['price'] ?? 0) * ($service['qty'] ?? 0),
					"taxRateNomenclature" => [null => null, '0' => null, '10' => 10, '20' => 20][$serviceDB['servicesVat']],
					"totalVat" => -1 * ((($service['price'] ?? 0) * ($service['qty'] ?? 0)) / (1 + (([null => null, '0' => null, '10' => 10, '20' => 20][$serviceDB['servicesVat']] ?? 0)) / 100) - (($service['price'] ?? 0) * ($service['qty'] ?? 0)))
				];
			}
		}
	}//процедурки
	...
	$SBIS["companyID"] = "130";
	$SBIS["kktRegNumber"] = ($_JSON['kkt']['regId'] ?? null); //
	$SBIS["cashierFIO"] = "КАССИР";
	...
	$SBIS["customerPhone"] = null;
	$SBIS["customerINN"] = null;
	$SBIS["customerExtId"] = null;
	$SBIS["taxSystem"] = "1";
	...
	sendTelegram('sendMessage', ['chat_id' => 325908361, 'text' => '' . json_encode($SBIS, 288 + 128)]);

	if (($_JSON['payments']['cash']['value'] ?? 0) || ($_JSON['payments']['card']['value'] ?? 0)) {

		if ($_JSON['sale']['entity'] == 1 && DBNAME == 'warehouse') {
			...
			if (1) {
				...
				curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type:application/json', 'Authorization: Bearer 7e2eaef46cc3369bb0a06dea5007150e']);
...
//рассрочка
	if ($_JSON['payments']['installment']['enabled'] ?? false) {
		mysqlQuery("INSERT INTO `f_installments` SET  "
				...
				. "`f_installmentsSumm`='" . mres($_JSON['payments']['card']['value']) . "'");
	}
