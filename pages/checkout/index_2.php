	...
	<div id="vueapp" style="background-color: white; border-radius: 8px;">
		...
		<div style=" padding: 20px;">
			<ul class="horisontalMenu">
				...
				<li v-on:click="saveSale" v-if="clientOk && saleOk && paymentsOk && personnelOk"><a style=" background-color: lightgreen;">Оформить</a></li>
		...
		<div v-if="tab=='sale'" class="box neutral" style="min-width: 1000px;">
			<div class="box-body">
						...
						<td style="vertical-align: top; padding: 20px;">
							...
							<div style=" display: inline-block;">
								<div class="lightGrid" style="display: grid; grid-template-columns: repeat(2, auto);">
									...
									<div style="display: contents;">
										<div style="display: flex; align-items: center;">Тип договора <span v-if="!data.sale.type" style="color: red; font-weight: bold;">*</span></div>
										<div><select id="saleType" v-model="data.sale.type" autocomplete="off">
												<option value=""></option>
						...
						<td style=" vertical-align: top; padding: 20px;">
							...
							<div style=" display: inline-block;">
								<div v-if="plans.length>0" class="lightGrid" style="display: grid;grid-template-columns: repeat(4, auto);">
									...
									<div v-for="(plan,index) in plans" v-on:click="planView(index);" style="display: contents; cursor: pointer; ">
				...
				<div class="lightGrid" v-if="plan.idf_salesDraft" style="display: grid; grid-template-columns: auto 1fr repeat(4, auto);">
					...
					<div v-for="(subscriptionDraft, index) in plan.f_subscriptionsDraft" style="display: contents;" :style="{color: (subscriptionDraft.f_subscriptionsDraftSalesSale?`gray`:`black`)}">
						<div class="R">{{index+1}}.</div>
						<div>{{subscriptionDraft.servicesName}}</div>
						<div class="C">{{subscriptionDraft.qty}}</div>
						<div class="R">{{subscriptionDraft.price}}р.</div>
						<div class="R">{{subscriptionDraft.qty*subscriptionDraft.price}}р.</div>
						<div class="C">
							<i v-if="subscriptionDraft.f_subscriptionsDraftSalesSale" class="fas fa-check-circle" style="color: silver; box-shadow: 0px 0px 5px 1px hsla(0,0%,0%,0.3); border-radius: 50%;"></i>

							<i v-if="!subscriptionDraft.f_subscriptionsDraftSalesSale && !data.sale.subscriptions.find(o => o.idf_subscriptionsDraft === subscriptionDraft.idf_subscriptionsDraft)" v-on:click="planServiceMove(index);" class="fas fa-arrow-circle-down" style="color: green; cursor: pointer;"></i>
							<i v-if="!subscriptionDraft.f_subscriptionsDraftSalesSale && data.sale.subscriptions.find(o => o.idf_subscriptionsDraft === subscriptionDraft.idf_subscriptionsDraft)" class="fas fa-check-circle" style="color: greenyellow; box-shadow: 0px 0px 5px 1px hsla(0,0%,0%,0.3); border-radius: 50%;"></i>
						</div>
					</div>
					<div style="display: contents;">
				...
				<div class="lightGrid"  style="display: grid;grid-template-columns: auto 1fr repeat(6, auto);">
					<div style="display: contents;">
						<div class="B C">№<br>п.п.</div>
					...
					<div v-for="(subscription, index) in data.sale.subscriptions" style="display: contents;">
						...
						<div class="C" style="display: flex; align-items: center; justify-content: center;"><input style="width: 3em; text-align: center;"  :readonly="subscription.idservicesApplied || subscription.idf_subscriptionsDraft" type="text" v-model="subscription.qty"></div>
						<div class="C" style="display: flex; align-items: center; justify-content: center;"><input style="width:  6em; text-align: center;" :readonly="subscription.idservicesApplied || subscription.idf_subscriptionsDraft" type="text" v-model="subscription.price"></div>
					...
					<div style="display: contents;">
						<div style="grid-column: span 5; text-align: right; font-weight: bold;">Итого:</div>
				...
				<div style="padding: 5px;  display: inline-block;">
					<div style="border: 1px solid silver; border-radius: 10px; width: auto; padding: 15px 40px;" :style="{ backgroundColor: (paymentsTotal>0)?(( paymentsTotal==saleTotal)?`#EAFFEA`:`#FFEAEA`):`#FFF`}">
						<h3>Платежей на сумму {{nf(paymentsTotal)}}р. <span v-if="paymentsTotal!=saleTotal">
								{{paymentsTotal>saleTotal?(`излишек ${paymentsTotal-saleTotal}`):(`доплатить осталось ${saleTotal-paymentsTotal}`)}}р.
							</span></h3>
					</div>
				</div>
				<br>
