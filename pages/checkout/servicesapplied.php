...
//		printr($_POST);
	...
	if (
		...
		if (mysqlQuery("INSERT INTO `f_payments` SET "
						...
						. "`f_paymentsAmount` = '" . FSI($_POST['summ']) . "', "
						. "`f_paymentsUser` = '" . $_USER['id'] . "', "
						. "`f_paymentsClient` = null, "
...
//					var_dump($clientsTaxPersons);
					...
					if ($_GET['contract'] ?? 0) {
						...
						if ($contract) {
							?>
							<h3 style="margin-top: 30px; text-align: center;">Договор от <?= date("d.m.Y", $contract['f_salesDateTS']); ?></h3>
							<div style="display: grid; grid-template-columns: auto auto; grid-gap: 10px 10px; padding: 10px;">
								<div>Кредитный менеджер</div>
								<div><?= $contract['usersLastName'] ?? ''; ?> <?= $contract['usersFirstName'] ?? ''; ?></div>
												...
												<tr>
													<td style="vertical-align: top;">
														<? if (count(($replacenetsSummary2arr['removed'] ?? []))) { ?>
															...
															<div style="display: grid; grid-template-columns: auto auto auto auto; background-color: hsla(0,100%,95%,1);" class="lightGrid">
																<div style="display: contents;">
													...
													<td style="vertical-align: top;">
														<? if (count(($replacenetsSummary2arr['appended'] ?? []))) { ?>
															...
															<div style="display: grid; grid-template-columns: auto auto auto auto; background-color: hsla(120,100%,95%,1);" class="lightGrid">
																<div style="display: contents;">
							...
							<form action="<?= GR(); ?>" method="POST">
								<div style="display: grid; grid-template-columns: auto auto auto; grid-gap: 4px 10px; padding: 10px;">
									...
									foreach (($f_credits ?? []) as $f_credit) {
										...
										<div style="text-align: center;"><?= $f_credit['RS_banksName']; ?></div>
										<?
									...
									foreach ($payments as $payment) {
										?>
										<div><?= $payment['f_paymentsDateTS'] ? date("d.m.Y", $payment['f_paymentsDateTS']) : 'Не помню...'; ?></div>
										<div style="text-align: right;"><?= nf($payment['f_paymentsAmount'], 2); ?>р.</div>
										<div style="text-align: center;"><?= $payment['f_paymentsTypesName'] ?? 'Не знаю...'; ?></div>
										<?
...
//						printr($services);
						...
						$contracts = query2array(mysqlQuery("SELECT * FROM `f_sales` LEFT JOIN `entities` ON (`identities` = `f_salesEntity`)  WHERE `f_salesClient`='" . $client['idclients'] . "'"));
//						printr($contracts);
						...
						<div>
							<? if (count($contracts)) { ?>
								...
								<div style="display: inline-block; padding: 10px;">
									<div class="lightGrid" style="display: grid; grid-template-columns: repeat(10,auto);">
										<div style=" display: contents;">
