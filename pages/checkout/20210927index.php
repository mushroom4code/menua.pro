...
if (!R(26)) {
	...
	<div style="padding: 10px;">
		...
		<br>
		<div style=" display: inline-block;">
			<div class="box neutral salesWindows" style="vertical-align: top; display: none;" id="salesDraft">
				<div class="box-body">
					<h2>Планы лечения</h2>
					<div class="lightGrid" id="salesDraftContent">
...
//								. "  AND (SELECT COUNT(1) FROM `positions2services` WHERE `positions2servicesService`=`idservices`)>0 "
				...
				<br><!-- comment -->
				<div style="display: inline-block;">
					<div class="lightGrid" id="subscriptions"  style="display: grid; grid-template-columns: auto auto 80px 80px 120px 120px 60px;"></div>
				</div>
				<br>
				<div style="display: inline-block; float: right; padding: 20px;">
					<div style="display: grid; grid-template-columns: repeat(2, auto); grid-gap: 10px 5px;">
						<div style="text-align: right;">Итого стоимость абонемента:</div>
						<div style=""><input style="text-align: right;" type="text" readonly autocomplete="off"  id="subscriptionTotalValue" placeholder="сумма"></div>
						<div style="text-align: right;">С учётом скидок:</div>
						<div style=""><input style="text-align: right;" type="text" readonly autocomplete="off"  id="subscriptionTotalPay" placeholder="сумма" oninput="digon(); sale.payment.summ=+this.value;"></div>
					</div>
				</div>
		...
		<div style=" display: inline-block;">
			...
			<br>
			<input type="button" id="savebutton" value="Сохранить" onclick="saveSale();">
			<br>
			<br>
							...
							?>'),
								...
								async function saveSale() {
									...
									if ((
										...
										MSG('По банку там чего-то не то...');
										fetch('/sync/api/icq/jse.php', {body: JSON.stringify({
												e: 'Косяк по банку',
												payment: sale.payment.bank
									...
									if (a != b)
									{
										MSG('Давай на секундочку остановимся<br> и проверим платежи. <br>Банк, нал или рассрочка.<br> Точно всё заполнено?');
										fetch('/sync/api/icq/jse.php', {body: JSON.stringify({
												e: 'Чуть не отправила продажу с кривыми данными по оплате. Аллилуя я спасла 😂',
