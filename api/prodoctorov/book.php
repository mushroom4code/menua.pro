...
//try to filter by name
	...
	if (trim($clients[0]['clientsLName']) == trim($_JSON['client']['last_name'] ?? '') && trim($clients[0]['clientsFName']) == trim($_JSON['client']['first_name'] ?? '')) {
		$client = $clients[0];
	} else {
...
//			. " `clientsAddedBy`='707', "
	...
	$client = mfa(mysqlQuery("SELECT * FROM `clients`"
					. " LEFT JOIN `clientsPhones` ON (`clientsPhonesClient` = `idclients`)"
...
$smsTemplatesText = mfa(mysqlQuery("SELECT * FROM `smsTemplates` WHERE `smsTemplatesDeleted` = '2022-02-25 10:59:03'"))['smsTemplatesText'] ?? null;

$smsdata = [
	'dateone' => date("d.m", strtotime($appointment['dt_start'])),
	'timeone' => date("H:i", strtotime($appointment['dt_start'])),
];
$smsText = smsTemplate($smsTemplatesText, $smsdata);

$sendResult = sendSms(($phoneNumber ?? null), $smsText);
$success = (($sendResult['status'] ?? '') === 'ok');
if ($success) {
	$uid = preg_replace("/message-id-/", '', $sendResult['result']['uid']);
	mysqlQuery("UPDATE `clientsPhones` SET `clientsPhonesSmsTotal` = `clientsPhonesSmsTotal`+1 WHERE `idclientsPhones` = '" . $client['idclientsPhones'] . "'");
	mysqlQuery("INSERT INTO `sms` SET "
			. "`smsHash` = '" . $uid . "', "
			. "`smsClient` = '" . $client['idclients'] . "', "
			. "`smsText` = '" . mres($smsText) . "', "
			. "`smsPhone` = '" . $client['idclientsPhones'] . "'");
} else {
	$output['errors'][] = 'Не удалось отправить SMS';
	die(json_encode(["status_code" => 425, "detail" => "can't send sms"]));
} 
